...
- `Gimbal` 保留本工程已经工作的 QD4310 和 INS Topic 链路，并通过静态 Topic 接收
  命令、发布低频反馈。
- `Chassis` 保留 Meta 的四舵轮运动学和最短转向逻辑，电机接口已改为本工程的
  `Class_DJIMotor`。机械尺寸和舵向零位是待标定参数，默认 `CHASSIS=0`。
- `Shoot` 的摩擦轮、反转和连发目标使用 Topic；单发、三发通过固定容量事件 FIFO
  触发。依赖裁判数据和实机阈值的热量/卡弹判断未移植，默认 `SHOOT=0`。
...
1. 静态 Topic、Publisher、Subscriber 和 EventQueue 在启动前已构造，无需注册。
2. `Control_Task` 启动后初始化各 Application 拥有的设备。
3. 每个控制周期依次调用 `RobotCmd_Update()`、`Gimbal_Update()`、
   `Chassis_Update()`、`Shoot_Update()`。

连续命令只在内容改变时发布，应用反馈降频到100 Hz。1 kHz INS -> Gimbal 状态链继续
使用 `Topic<INS_State>`，不增加任务切换或阻塞。
